...
![img.png](img.png)
В терминале с go-приложением выводится отправленное сообщение, значит всё работает! Таким образом, на данном этапе мы смогли принять сегмент от канального уровня и провести его через Kafka. Давайте разберемся теперь со сборкой сегментов в сообщение.
...
Для начала напишем функцию сегментации. Нужно учесть, что длина сообщения может быть не кратна M:
```go
...
Проверим работу метода получения и сегментации сообщения. Через postman вызовем метод /send, и убедимся, что в консоль вывелось соощение, разбитое на сегменты:
![img_5.png](img_5.png)
![img_6.png](img_6.png)
...
И, наконец, закончим написание хранилища - напишем функцию ScanStorage, который будет склеивать нужные сообщения и отправлять на прикладной уровень:
```go
const (
...
- Реализовали получение сообщения от прикладного уровня с последующей его сегментаций и отправкой на канальный уровень.
